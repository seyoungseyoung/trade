...
# SQQQ 전환 전략 구현
def implement_switching_strategy(df, models, params):
    """TQQQ/SQQQ 전환 전략 구현"""
...
# 결과 시각화
def visualize_results(df, performance, output_dir='results'):
    """백테스팅 결과 시각화"""
    if df is None:
        return
    ...
    plt.plot(df.index, 1 + df['cum_strategy_return'], 'g-', label='Strategy')
    ...
    plt.plot(df.index, df['strategy_capital'], 'g-', label='Strategy')
    ...
    plt.plot(df.index, df['cum_strategy_return'] * 100, 'g-', label='Strategy')
    ...
    plt.plot(df.index, strategy_drawdown, 'g-', label='Strategy')
